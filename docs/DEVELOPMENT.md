...
3. **Service implementation** – The server was built in Python with a lightweight dependency set using MCPEngine for HTTP-based communication. Network access to CKAN is encapsulated in `ckan.py`, while tool orchestration lives in `lambda_server.py`. Configuration parameters are centralized in `config.py` so environments can tune limits without code changes.
...
5. **Validation with MCP clients** – The team exercised the HTTP server with MCP-compatible clients such as Claude Desktop via MCPEngine proxy, iterating on tool descriptions, error messaging, and performance safeguards until the integration felt reliable.
...
- **`servers/boston_opendata_lambda/lambda_server.py`** – Main server code using MCPEngine, defines available tools, and handles HTTP-based MCP communication. Can run locally for development or deploy to AWS Lambda.
- **`servers/boston_opendata_lambda/ckan.py`** – Handles HTTP interactions with Boston's CKAN API, including retries and response validation.
- **`servers/boston_opendata_lambda/utils/formatters.py`** – Shapes dataset metadata and query results into concise structures suitable for AI consumption.
- **`servers/boston_opendata_lambda/config.py`** – Centralizes configuration constants such as the CKAN base URL, timeouts, and maximum record counts using Pydantic settings.
- **`servers/boston_opendata_lambda/utils/`** – Utility modules for logging, validation, rate limiting, and circuit breaking.
